...
function loadNewWord() {
  ...
  if (!isHardMode && gameMode !== "survival") {
    document.getElementById("categoryHint").innerHTML =
      "<p>📂 Kategori: <strong>" + currentWordObj.category + "</strong></p>";
  } else {
    document.getElementById("categoryHint").innerHTML = "";
  }
...
function guessLetter() {
  ...
  if (!input) {
    showMessage("❌ Masukkan huruf dulu!", "red");
    return;
  }

  if (!/^[a-z]$/.test(input)) {
    showMessage("❌ Hanya huruf a-z saja ya!", "red");
    document.getElementById("guessInput").value = "";
    return;
  }
  ...
  if (guessedLetters.includes(input)) {
    showMessage(
      '⚠️ Huruf "' + input.toUpperCase() + '" sudah kamu tebak!',
      "orange"
    );
    document.getElementById("guessInput").value = "";
    return;
  ...
  if (found) {
    ...
    showMessage(
      '❌ Salah! Huruf "' + input.toUpperCase() + '" tidak ada!',
      "red"
    );
    lives--;
    wrongGuesses++;
...
function guessFullWord() {
  ...
  if (!input) {
    showMessage("❌ Masukkan kata lengkap!", "red");
    return;
  }
  ...
  if (input === currentWordObj.word.toLowerCase()) {
    ...
    showMessage("🎉 BENAR SEMPURNA! +" + (100 + timeBonus) + " poin!", "green");
    wordCompleted();
  } else {
    attemptsLeft--;
    wrongGuesses++;
    ...
    if (attemptsLeft <= 0) {
      ...
      showMessage("❌ Salah! Sisa " + attemptsLeft + " kesempatan", "red");
    }
  }
...
function useHintBonus() {
  if (hintBonusCount <= 0) {
    showMessage("❌ Bonus hint sudah habis!", "red");
    return;
  }
  ...
  if (hiddenIndices.length === 0) {
    showMessage("⚠️ Semua huruf sudah terbuka!", "orange");
    return;
  }
  ...
  hintBonusCount--;
  showMessage(
    '💡 Hint: Huruf "' + revealedLetter.toUpperCase() + '" terbuka!',
    "blue"
  );
...
function wordCompleted() {
  ...
  if (streak >= 3) {
    ...
    showMessage('🎉 Benar! Kata: "' + currentWordObj.word + '"', "green");
  }
  ...
  if (wordsCompleted % 3 === 0 && gameMode !== "speedrun") {
    ...
    if (gameMode !== "survival") {
      ...
      showMessage(
        "🎊 LEVEL " + level + "! Bonus: +1 Nyawa & +1 Hint!",
        "purple"
      );
    } else {
      showMessage("🎊 LEVEL " + level + "! Kamu masih bertahan!", "purple");
    }
  }

  if (gameMode === "speedrun") {
    timeRemaining += 10;
    showMessage("⏱️ +10 detik bonus!", "blue");
  }
...
function skipWord() {
  lives--;
  streak = 0;
  showMessage('⏭️ Skip! Kata: "' + currentWordObj.word + '"', "gray");
...
function giveUp() {
  showMessage('🏳️ Menyerah! Kata: "' + currentWordObj.word + '"', "gray");
  setTimeout(gameOver, 2000);
}
